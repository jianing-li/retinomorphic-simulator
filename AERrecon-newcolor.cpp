...
int main(int argc, char** argv)
{
	...
	while (1)
	{
		...
		c = getopt_long(argc, argv, "w:h:n:f:i:s:e:o:", long_options, &option_index);
		if (c == -1)
			break;
		switch (c)
		{
	...
	lastDVStime = new double[Width * Height];

	memset(lastDVStime, 0, sizeof(double) * Width * Height);
	...
	if (UseTime)
	{
		...
		for (int i = 1;; i += TimeperImage)
		{
			...
			while(1)
			{
				...
				if (fabs(t - lastDVStime[x * Height + y]) < timewin)
				{
					if (p == 1)
					{
...
void Generateimage(int* eventcnt, int Width, int Height)
{
	...
	char buffer[256];
	sprintf_s(buffer, 256, ("G:\\recon\\DVSrecon\\" + dirname + "\\DVSlikeRecon%05d.png").c_str(), cnt + start_idx);
